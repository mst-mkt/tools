...
export const Route = createFileRoute('/_layout/formatter/sql')({
...
const Sql = () => {
  ...
  const formattedCode = useMemo(() => {
    try {
      const formatted = format(code, 'text.sql').trim()
  ...
  return (
    <>
      <Head title="SQL Formatter" />
      <div className="flex flex-col gap-y-4">
        <h1 className="font-bold text-lg">SQL Formatter</h1>
        <Textarea value={code} onChange={setCode} placeholder="Enter SQL Query..." />
        <div className="scrollbar-thin scrollbar-thumb-background-200 scrollbar-thumb-rounded-full scrollbar-track-transparent overflow-y-hidden overflow-x-scroll rounded-md border border-background-100 p-4 text-sm">
          <Code lang="sql" code={formattedCode} />
        </div>
