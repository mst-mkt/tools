...
export const Route = createFileRoute('/_layout/formatter/yaml')({
...
const Yaml = () => {
  ...
  const formattedCode = useMemo(() => {
    try {
      const formatted = format(code, 'text.yaml').trim()
  ...
  return (
    <>
      <Head title="Yaml Formatter" />
      <div className="flex flex-col gap-y-4">
        <h1 className="font-bold text-lg">Yaml Formatter</h1>
        <Textarea value={code} onChange={setCode} placeholder="Enter yaml text..." />
        <div className="scrollbar-thin scrollbar-thumb-background-200 scrollbar-thumb-rounded-full scrollbar-track-transparent overflow-y-hidden overflow-x-scroll rounded-md border border-background-100 p-4 text-sm">
          <Code lang="yml" code={formattedCode} />
        </div>
